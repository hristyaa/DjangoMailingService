...
- Навигация по сайту (для авторизованных пользователей: "Главная", "Клиенты", "Сообщения", "Расслылки", "Попытки
  рассылки", "Редактирование профиля", "Выход"; для неавторизованных пользователь: "Главная", "Вход", "Регистрация") ;
...
- Кнопка "Добавить нового клиента" -> возможность добавления нового клиента;
...
- Кнопка "Добавить новое сообщение" -> возможность добавления нового сообщения;
...
- Кнопка "Создать рассылку" -> возможность создать рассылку;
...
В проекте доступны готовые фикстуры для быстрого запуска тестовых данных:
- `fixtures/clients.json` - тестовые получатели рассылок;
- `fixtures/messages.json` - тестовые сообщения;
- `fixtures/mailing.json` - тестовые рассылки; 
- `fixtures/users.json` - тестовые пользователи;
- `fixtures/attempt.json` - тестовые попытки отправки рассылок.

Загрузка всех фикстур:
```
python manage.py loaddata fixtures/clients.json fixtures/messages.json fixtures/mailing.json fixtures/users.json fixtures/attempt.json
```
...
class Mailing(models.Model):
    ...
    recipients = models.ManyToManyField(
        MailingRecipient, blank=True, verbose_name="Получатели"
    )
...
class AttemptSendMailing(models.Model):
    ...
    recipient = models.ForeignKey(
        MailingRecipient, on_delete=models.CASCADE, verbose_name="Получатель"
    )

    def __str__(self):
        return (
            f"{self.mailing}, {self.recipient}: {self.status} - {self.server_response}"
        )
